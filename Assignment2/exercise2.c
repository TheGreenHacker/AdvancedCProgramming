...
{                                                                         \
    ...
    unsigned int i;                                                       \
    for (i = 0; i < MAX_NBRS && (nbrnode = node->nbrs[i]) != NULL; i++)   \
...
{                                                                                        \
    ...
    node_t *_nbrnode;                                                                    \
    unsigned int i;                                                                      \
    for (i = 0; i < MAX_NBRS && (_nbrnode = node->nbrs[i]) != NULL; i++){                \
        unsigned int j;                                                                  \
        node_t *_nbrnbrnode;                                                             \
        for (j = 0; j < MAX_NBRS && (_nbrnbrnode = _nbrnode->nbrs[j]) != NULL; j++){     \
            if (!strncmp(node->node_name, _nbrnbrnode->node_name, NAME_SIZE)) continue;  \
            nbrnode = _nbrnbrnode;                                                       \
...
/*Problem 2  : In thie Question, I have created a graph as shown in the diagram below.
 printf("%s, %s\n", _nbrnode->node_name, _nbrnbrnode->node_name);      \*/
